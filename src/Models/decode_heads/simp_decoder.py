...
class SimpleDecoder(BaseDecodeHead):
    """
    This head is modified from
...
class DAR_ditill_head(nn.Module):
    def __init__(self, 
            dim,         
            predict_feature_dim: int = 768,
                 
        ) -> None:
        super().__init__()
        
        self.mlp = nn.Sequential(
            nn.Linear(dim, predict_feature_dim),
            nn.ReLU(inplace=True),
            nn.Linear(predict_feature_dim, predict_feature_dim),
        )
