...
try:
    if XFORMERS_ENABLED:
        from xformers.ops import (fmha, scaled_index_add, index_select_cat, 
                                  SwiGLU, memory_efficient_attention, unbind)
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, extra_return = False) -> torch.Tensor:
        B, N, C = x.shape
    ...
    def _get_attn_weights(self, x: torch.Tensor) -> torch.Tensor:
        B, N, C = x.shape
...
class MemEffAttention(Attention):
    def forward(self, x: torch.Tensor, attn_bias=None, extra_return=False) -> torch.Tensor:
        if not XFORMERS_AVAILABLE:
        ...
        if extra_return:
            q_cache, k_cache = q.permute(0, 2, 1, 3) * self.scale, k.permute(0, 2, 1, 3) # (B, n_h, N, c)
            attn = q_cache @ k_cache.transpose(-2, -1)
            attn = attn.softmax(dim=-1)
            # (B, num_heads, N, N)
            attn_weights = attn
...
class Block(nn.Module):
    ...
    def get_attn_weights(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.attn._get_attn_weights(self.norm1(x))
